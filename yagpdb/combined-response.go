{{/* https://docs.yagpdb.xyz/reference/ets#ets.sdict */}}
...
{{ $te := sdict "color" 2003199 }}
{{ $rt := .ExecData.message }}
{{ $ar := false }}
{{ $modmaillogo := "702099194701152266" }}
{{ $discordlogo := "579210587557462021" }}
{{ $cb := cslice (cbutton "label" "Delete Response" "custom_id" "mmsr-delete" "style" 4 "emoji" (sdict "id" "1251255316121653343")) (cbutton "label" "Bookmark Response" "custom_id" "mmsr-bookmark" "style" 2 "emoji" (sdict "id" "1251243802207846566")) }}
{{ $eb := cslice }}
{{ $rm := cslice (sdict) }}
...
{{ if eq .ExecData.trigger 2 3 }}
	...
	{{ if eq .ExecData.trigger 2 }}
		{{ $re.Set "title" "How do I get unbanned?" }}
		{{ $re.Set "description" "You cannot use ModMail to contact a server you are banned from." }}
	{{/* Add "wrong server" specific title and description */}}
	{{ else if eq .ExecData.trigger 3 }}
		{{ $re.Set "title" "How do I contact *XYZ* Server" }}
		{{ $re.Set "description" "Please DM the __**bot**__ <@575252669443211264> with your message instead. Make sure __**not**__ to select __ModMail Support__ as that will send it to this server" }}
	{{ end }}
	{{ $re.Set "fields" (cslice (sdict 
	...
	{{ rm.Append (sdict "re" $re "cb" $cb) }}
	{{ sendMessageNoEscape nil (complexMessage "reply" $rt "embed" $re "buttons" $cb) }}
	{{ $ar := true }}
{{ end }}
...
{{ if eq .ExecData.trigger 4 }}
	...
	{{ range $k, $v := $te }}
		{{ $re.Set $k $v}}
	{{ end }}
	{{ $re.Set "title" "How do I setup ModMail?" }}
	{{ $re.Set "fields" (cslice (sdict 
			"name" "Initial Setup"
	...
	{{ $cb := $cb.Append (cbutton "label" "Toggle Extra Information" "custom_id" "mmsr-toggle" "style" 1 "emoji" (sdict "id" "1258858981372330165")) }}
	{{ $eb = $eb.AppendSlice (cslice (cbutton "label" "Invite ModMail" "custom_id" "mmsr-invite" "url" "https://modmail.xyz/invite" "style" "link" "emoji" (sdict "id" $modmaillogo)) (cbutton "label" "ModMail Commands (=help)" "custom_id" "mmsr-commands" "url" "https://modmail.xyz/commands" "style" "link" "emoji" (sdict "id" $modmaillogo ))) }}
	{{ $rm = rm.Append (sdict "re" $re "cb" $cb "ee" $eb) }}
	{{ sendMessageNoEscape nil (complexMessage "reply" $rt "embed" $re "buttons" $cb "buttons" $eb) }}
	{{ $ar := true }}
{{ end }}
...
{{ if eq .ExecData.trigger 5 }}
	...
	{{ range $k, $v := $te }}
		{{ $re.Set $k $v}}
	{{ end }}
	{{ $re.Set "title" "How do I open a ticket?" }}
	{{ $re.Set "fields" (cslice (sdict 
		...
		{{ $cb = $cb.Append (cbutton "label" "Toggle Extra Information" "custom_id" "mmsr-toggle" "style" 1 "emoji" (sdict "id" "1258858981372330165")) }}
	{{ $rm = $rm.Append (sdict "re" $re "cb" $cb) }}
	{{ sendMessageNoEscape nil (complexMessage "reply" $rt "embed" $re "buttons" $cb) }}
	{{ $ar := true }}
{{ end }}
...
{{ if eq .ExecData.trigger 6 }}
	...
	{{ range $k, $v := $te }}
		{{ $re.Set $k $v}}
	{{ end }}
	{{ $re.Set "title" "Donation Link" }}
	{{ $re.Set "description" "[Purchase ModMail Premium Here](https://modmail.xyz/premium)" }}
	{{ $eb = $eb.Append (cbutton "label" "Buy Premium" "custom_id" "mmsr-premium" "url" "https://modmail.xyz/premium" "style" "link" "emoji" (sdict "id" $modmaillogo)) }}
	{{ $rm = $rm.Append (sdict "re" $re "cb" $cb "eb" $eb) }}
	{{ sendMessageNoEscape nil (complexMessage "reply" $rt "embed" $re "buttons" $cb) }}
	{{ $ar := true }}
{{ end }}
...
{{ if eq .ExecData.trigger 11 }}
	...
	{{ range $k, $v := $te }}
		{{ $re.Set $k $v}}
	{{ end }}
	{{ $re.Set "title" "Advanced Logging Example" }}
	{{ $re.Set "description" "This is an example of what you'll get with advanced logging. https://modmail.xyz/logs/d7586c153425000-10d1416086c01033-10d141608b802047" }}
	{{ $f := "[2024-02-26 23:44:18] scyye#0 (User): Hello! I'm a user in need of assistance, can someone help me?\n[2024-02-26 23:44:43] scyye#0 (Comment): I am a staff member writing a comment on the ticket, for other staff to see\n[2024-02-26 23:45:16] scyye#0 (Staff): I am now replying to the user, asking them what they need help with.\n[2024-02-26 23:45:46] jrwallor#0 (Staff): Another staff member with an anonymous reply.\n[2024-02-26 23:45:58] scyye#0 (User): This is the user replying, thanking me for support (I didn't think this through, cut me some slack)\n[2024-02-26 23:46:36] scyye#0 (Comment): =c This ticket is resolved, so I'm closing it now." }}
	{{ $eb = $eb.Append (cbutton "label" "Example Logs" "custom_id" "mmsr-logs" "url" "https://modmail.xyz/logs/d7586c153425000-10d1416086c01033-10d141608b802047" "style" "link" "emoji" (sdict "id" $modmaillogo)) }}
	{{ $rm = $rm.Append (sdict "re" $re "cb" $cb "eb" $eb "f" $f) }}
	{{ sendMessageNoEscape nil (complexMessage "reply" $rt "embed" $re "file" $file) }}
	{{ $ar := true }}
{{ end }}
...
{{ if eq .ExecData.trigger 7 }}
	...
	{{ range $k, $v := $te }}
		{{ $re.Set $k $v}}
	{{ end }}
	{{ $re.Set "title" "ModMail is not responding" }}
	{{ $re.Set "description" "If ModMail is not responding in your server, please check the following:\n- The bot has Read Messages, Send Messages, and re Links permissions.\n- You are using the correct prefix. Use `@ModMail prefix` to check the prefix.\n- The command you are using is valid. Check using `=help <command>`.\n- The bot is online. Discord might be having issues, or the bot might be restarting.\n\nIf the bot still does respond, please let us know your [server ID](https://support.discord.com/hc/en-us/articles/206346498-Where-can-I-find-my-User-Server-Message-ID-)." }}
	{{ $eb = $eb.Append (cbutton "label" "Discord ID Guide" "custom_id" "mmsr-idguide" "url" "https://support.discord.com/hc/en-us/articles/206346498-Where-can-I-find-my-User-Server-Message-ID-" "style" "link" "emoji" (sdict "id" $discordlogo)) }}
	{{ $rm = $rm.Append (sdict "re" $re "cb" $cb "eb" $eb) }}
	{{ sendMessageNoEscape nil (complexMessage "reply" $rt "embed" $re "buttons" $cb) }}
	{{ $ar := true }}
{{ end }}
...
{{ if eq .ExecData.trigger 8 }}
	...
	{{ range $k, $v := $te }}
		{{ $re.Set $k $v}}
	{{ end }}
	{{ $re.Set "title" "ModMail Custom Instance" }}
	{{ $re.Set "fields" (cslice (sdict 
...
{{ if eq .ExecData.trigger 10 }}
	...
	{{ range $k, $v := $te }}
		{{ $re.Set $k $v}}
	{{ end }}
	{{ $re.Set "title" "My message wasn't delivered!" }}
	{{ $re.Set "description" "If you receive \"your message could not be delivered\", check your privacy settings for the server you want to contact. You need to enable the \"allow direct messages from server members\" option." }}
	{{ $re.Set "image" (sdict "url" "https://i.imgur.com/x5Hcio5.png") }}
	{{ $rm = $rm.Append (sdict "re" $re "cb" $cb) }}
	{{ sendMessageNoEscape nil (complexMessage "reply" $rt "embed" $re "buttons" $cb) }}
	{{ $ar := true }}
	{{ end }}
...
{{ if eq .ExecData.trigger 11 }}
	...
	{{ range $k, $v := $te }}
		{{ $re.Set $k $v}}
	{{ end }}
	{{ $re.Set "title" "Everyone can see my tickets!" }}
	{{ $re.Set "description" "If everyone in your server is able to view tickets, there is a chance that another bot in your server is interfering. This is not a problem with ModMail. We recommend checking the audit logs in your server settings to see which bot is changing the channel permissions." }}
	{{ $re.Set "fields" (cslice (sdict
		"name" "Server Wide permissions"
		"value" "These permissions are found in: **Server Settings --> Roles**\n\n- Your `@everyone` role needs to have `Send Messages` and `view message history` toggled on and `view channel` toggled off.\n - **Optional:** It can also have `connect`, `speak`, voice activity, `re links`, `change nickname`, `add reaction`.\n  - These are optional as some people like to restrict some of these perms behind role rewards or only give them to server boosters."
		"inline" false
	) (sdict
...
{{ if eq .ExecData.trigger 13 }}
	...
	{{ range $k, $v := $te }}
		{{ $re.Set $k $v}}
	{{ end }}
	{{ $re.Set "title" "ModMail Verfication" }}
	{{ $re.Set "description" "If you are having trouble verifying, please check the following:" }}
	{{ $re.Set "image" (sdict "url" "https://media.discordapp.net/attachments/576764484065165313/985238549886562374/unknown.png") }}
	{{ $re.Set "fields" (cslice (sdict
...
{{ if eq .ExecData.trigger 14 }}
	...
	{{ range $k, $v := $te }}
		{{ $re.Set $k $v}}
	{{ end }}
	{{ $re.Set "title" "ModMail Permissions" }}
	{{ $re.Set "description" "If you are having trouble with ModMail, please check the following:" }}
	{{ $re.Set "image" (sdict "url" "https://media.discordapp.net/attachments/576764854673735680/863863546915979274/unknown-4.png") }}
	{{ $re.Set "fields" (cslice (sdict
		"name" "Note"
	...
	{{ $eb = $eb.Append (cbutton "label" "Discord Permissions FAQ" "custom_id" "mmsr-permissions" "url" "https://support.discord.com/hc/en-us/articles/206029707-Setting-Up-Permissions-FAQ" "style" "link" "emoji" (sdict "id" $discordlogo)) }}
	{{ $rm = $rm.Append (sdict "re" $re "cb" $cb "eb" $eb) }}
	{{ sendMessageNoEscape nil (complexMessage "reply" $rt "embed" $re "buttons" $cb "buttons" $eb) }}
	{{ $ar := true }}
{{ end }}
...
{{ if eq .ExecData.trigger 15 }}
	...
	{{ range $k, $v := $te }}
		{{ $re.Set $k $v}}
	{{ end }}
	{{ $re.Set "title" "ModMails Raw Invite Link" }}
	{{ $re.Set "description" "[https://discord.com/oauth2/authorize?client_id=575252669443211264&permissions=268823640&response_type=code&redirect_uri=https%3A%2F%2Fmodmail.xyz%2Fwelcome&scope=bot+applications.commands](https://discord.com/oauth2/authorize?client_id=575252669443211264&permissions=268823640&response_type=code&redirect_uri=https%3A%2F%2Fmodmail.xyz%2Fwelcome&scope=bot+applications.commands)" }}
	{{ $eb = $eb.Append (cbutton "label" "ModMail Raw Invite" "custom_id" "mmsr-invite" "url" "https://discord.com/oauth2/authorize?client_id=575252669443211264&permissions=268823640&response_type=code&redirect_uri=https%3A%2F%2Fmodmail.xyz%2Fwelcome&scope=bot+applications.commands" "style" "link" "emoji" (sdict "id" $modmaillogo)) }}
	{[ $rm = $rm.Append (sdict "re" $re "cb" $cb "eb" $eb)]}
	{{ sendMessageNoEscape nil (complexMessage "reply" $rt "embed" $re "buttons" $cb "buttons" $eb) }}
	{{ $ar := true }}
{{ end }}
...
{{ if eq .ExecData.trigger 16 }}
	...
	{{ range $k, $v := $te }}
		{{ $re.Set $k $v}}
	{{ end }}
	{{ $re.Set "title" "ModMail is not responding, not sending my messages, cannot find my servers!" }}	
	{{ $re.Set "description" "ModMail is experiencing issues at this time, no ETA. It will be fixed when the Developer is available." }}
	{{ $re.Set "thumbnail" (sdict "url" "https://cdn.discordapp.com/avatars/575252669443211264/7050131180642ef969d1ac28bd7354b6.png?size=1024") }}
	{{ $rm = $rm.Append (sdict "re" $re "cb" $cb) }}
	{{ sendMessageNoEscape nil (complexMessage "reply" $rt "embed" $re "buttons" $cb) }}
	{{ $ar := true }}
{{ end }}
...
{{ if eq .ExecData.trigger 18 }}
	...
	{{ range $k, $v := $te }}
		{{ $re.Set $k $v}}
	{{ end }}
	{{ $re.Set "title" "ModMail Status" }}
	{{ $re.Set "description" "You can view the [bot status page](https://modmail.xyz/status) to see if there is any known outage." }}
	{{ $eb = $eb.Append (cbutton "label" "ModMail Status" "custom_id" "mmsr-status" "url" "https://modmail.xyz/status" "style" "link" "emoji" (sdict "id" $modmaillogo)) }}
	{{ $rm = $rm.Append (sdict "re" $re "cb" $cb "eb" $eb) }}
	{{ sendMessageNoEscape nil (complexMessage "reply" $rt "embed" $re "buttons" $cb "buttons" $eb) }}
	{{ $ar := true }}
{{ end }}
...
{{ if and ( eq .ExecData.trigger 1 ) ( not $ar) }}
	{{ $re := sdict }}
	{{ range $k, $v := $te }}
		{{ $re.Set $k $v}}
	{{ end }}
	{{ $re.Set "title" "Don't just say `i need help`, tell us what you need help with!" }}
	{{ $re.Set "description" "[This saves all of us time and we can jump in to provide you with a solution!](https://dontasktoask.com/)" }}
	{{ $rm = $rm.Append (sdict "re" $re "cb" $cb) }}
	{{ sendMessageNoEscape nil (complexMessage "reply" $rt "embed" $re "buttons" $cb) }}
{{ end }}
...
{{ if $rm }}
	{{ range $rm }}
		{{ sendMessageNoEscape nil (complexMessage "reply" $rt "embed" .re "buttons" .cb "buttons" .eb "file" .f) }} 